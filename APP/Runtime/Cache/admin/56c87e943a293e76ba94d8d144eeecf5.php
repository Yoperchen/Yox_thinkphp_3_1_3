...
<div class="Yocms_list_90" id="Yocms_list_goods_90">
<div class="header">
	<span style="color:#357ebd;font-size:15px;width:200px;float:left;font-family: cursive;">商品</span>
	<span class="close" id="Yocms_list_goods_90_toggle" title="关闭">x</span>
</div>
<ul>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><li data-goods-id="<?php echo ($goods['id']); ?>"><a title="<?php echo ($goods['name']); ?>" data-goods-id="<?php echo ($goods['id']); ?>" href="<?php echo U('Goods/goods_detail@Common',array('goods_id'=>$goods['id']));?>" target="_blank"><?php echo ($goods['name']); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
</ul>
<input id="goods_id" type="hidden" name="goods_id" value="0">
...
$("#Yocms_list_goods_90_toggle,.Yocms_list_90_toggle,#Yocms_list_goods_90_close,.Yocms_list_90_close,#Yocms_list_goods_90_open,.Yocms_list_90_open").live("click",function(event){
	$("#Yocms_list_goods_90").css("top",$(document).scrollTop()+50);
	$("#Yocms_list_goods_90").css("left",($(window).width())/4);
	$("#Yocms_list_goods_90").css("position","absolute");
	$("#Yocms_list_goods_90").toggle();
})
</script>
<script>
$("#Yocms_list_goods_90 ul li").live("click",function(){
	$("#Yocms_list_goods_90 #goods_id").val($(this).attr("data-goods-id"));
})
</script>