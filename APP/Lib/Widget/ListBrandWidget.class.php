<?php
//品牌列表
class ListBrandWidget extends Widget{  
    public function render($data){ 
    	$data['application'] = isset($data['application'])?$data['application']:'Common';
    	$Template = '';//模版
    	$Template = isset($data['template'])?$data['template']:'ListBrand';
    	$condition['site_id'] = $data['condition']['site_id']?$data['condition']['site_id']:'';//站点id
    	$condition['store_id'] = $data['condition']['store_id']?$data['condition']['store_id']:'';//商家id
    	$condition['user_id'] = $data['condition']['user_id']?$data['condition']['user_id']:'';//用户id
    	$condition['is_show'] = $data['condition']['is_show']?$data['condition']['is_show']:'1';//是否显示
    	...
    	$Brand_model  =   D('Brand');
    	$result = $Brand_model-> get_brand_list( $condition, $page_size=50);
