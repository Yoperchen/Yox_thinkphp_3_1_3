...
class FilmModel extends Model {
	 ...
	 * @return array
	 */
	public function get_film_by_id( $id=0,$isdetail=0){
		if(empty($id)){ 
			$data=array('status'=>0,'message'=>'id不能为空');
			return $data;
		}
		if($isdetail==0){
			$film_info = $this->where(array('id'=>$id))->find();
			$data=array('status'=>1,'message'=>'成功','data'=>$film_info);
			return $data;
		}else{//详细
			$film_info		 = $this->where(array('id'=>$id))->find();
			$film_detail_info = M('User')->where(array('id'=>$film_info['belong_user_id']))->find();
			$film_info		 =array_merge_recursive($film_info,$film_detail_info);
			$data=array('status'=>1,'message'=>'成功','data'=>$film_info);
			return $data;
		}
	 ...
	 * @param int $page_size
	 */
	public function get_film_list( $condition, $page_size=15){
...
// 		$this->assign('page',$show);// 赋值分页输出
	 ...
	 * @param array $data
	 */
	public function update_filmById($id, $data){
	 ...
	 * @param array $condition
	 */
	public function del_film($condition)
	{
