 ...
 */
class Film_play_sourceModel extends Model {
	/**
	 * 添加收藏
	 * @param array $data
	 */
	public function add_play_source( $data){
		$result=array('status'=>0,'message'=>'data不能为空');
		if(!empty($data)){
			$data['add_time']=time();
			 if($play_source_id=$this->data($data)->add()){
			 	$data['id']=$play_source_id;
	 ...
	 * @return array
	 */
	public function get_play_source_by_id( $id=0,$isdetail=0){
		if(empty($id)){ 
			$data=array('status'=>0,'message'=>'id不能为空');
			return $data;
		}
		if($isdetail==0){
			$play_source_info = $this->where(array('id'=>$id))->find();
			$data=array('status'=>1,'message'=>'成功','data'=>$play_source_info);
			return $data;
		}else{//详细
			$play_source_info		 = $this->where(array('id'=>$id))->find();
			$play_source_detail_info = M('User')->where(array('id'=>$play_source_info['belong_user_id']))->find();
			$play_source_info		 =array_merge_recursive($play_source_info,$play_source_detail_info);
			$data=array('status'=>1,'message'=>'成功','data'=>$play_source_info);
			return $data;
		}
	 ...
	 * @param int $page_size
	 */
	public function get_play_source_list( $condition, $page_size=15){
...
// 		$this->assign('page',$show);// 赋值分页输出
	 ...
	 * @param array $data
	 */
	public function update_play_sourceById($id, $data){
	 ...
	 * @param array $condition
	 */
	public function del_play_source($condition)
	{
