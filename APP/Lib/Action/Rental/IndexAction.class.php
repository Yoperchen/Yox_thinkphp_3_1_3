...
//    		$data['site_name']=;
    ...
    public function get_weather_info(){
	    ...
	    $callback = I('param.callback','', 'htmlspecialchars');
    	import('ORG.Weather',LIB_PATH);// 导入天气类
    	$Weather_obj = new Weather();
    	$District_model = D('District');
    	$city_info = $District_model-> getDistrictById(cookie('city_id'));
    	$weather_info_result = $Weather_obj->get_weather_info($city_info['data']['weather_area_id']);
		...
		if(empty($callback)){
			...
			exit($callback.'('.json_encode($weather_info_result).')');
		}
    	}
